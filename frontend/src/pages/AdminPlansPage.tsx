        ...
        <div className="grid gap-5 xl:grid-cols-2">
          {plans.map((plan) => (
            <section key={plan._id} className="space-y-4 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm">
              ...
              <label className="block text-sm text-slate-600">Tính năng sắp ra mắt (mỗi dòng hoặc phân cách bằng dấu ;)<textarea className="mt-1 min-h-24 w-full rounded-xl border border-slate-200 px-3 py-2" placeholder="Ví dụ: Trợ lý AI; Điểm tương thích với mô tả công việc" value={plan.SapRaMat} onChange={(e) => update(plan._id, 'SapRaMat', e.target.value)} /></label>
